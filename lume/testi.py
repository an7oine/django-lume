...
class Asiakas(Testimalli):
  ...
  pisin_osoite = lume.ForeignKey( # pylint: disable=no-member
    'Asiakasosoite', on_delete=models.DO_NOTHING,
    kysely=lambda: models.Subquery(
      Asiakasosoite.objects.filter(
...
class Asiakasosoite(Testimalli):
...
class Testi(test.TestCase):
  ...
  def setUpTestData(cls):
    ...
    Asiakasosoite.objects.create(asiakas=asiakas, osoite='Katu 1')
    Asiakasosoite.objects.create(asiakas=asiakas, osoite='Katu 123 B 4')
    Asiakasosoite.objects.create(asiakas=asiakas, osoite='Katu 456')
